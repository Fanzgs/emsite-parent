...
import com.empire.emsite.modules.gen.dao.GenDataBaseDictDao;
...
/**
 * 类GenTableService.java的实现描述：业务表Service
 * 
 * @author arron 2017年10月30日 下午4:26:43
 */
@Service
...
public class GenTableService extends BaseService {
    ...
    public GenTable getTableFormDb(GenTable genTable) {
        ...
        if (StringUtils.isNotBlank(genTable.getName())) {
            ...
            if (list.size() > 0) {
                ...
                // 初始化列属性字段
                GenDbsUtils.initColumnField(genTable);
    ...
    public void save(GenTable genTable) {
        ...
        User user = genTable.getCurrentUser();
        // 保存列
        for (GenTableColumn column : genTable.getColumnList()) {
            column.setGenTable(genTable);
            column.setCurrentUser(user);
